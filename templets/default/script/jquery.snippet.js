			...
			if(node == "pre"){
				...
				if(!o.parent().hasClass("snippet-wrap")){
					...
					var controls = "<div class='snippet-menu sh_sourceCode' style='display:none;'><pre>"
								  ...
								  +"<a class='snippet-window' href='#'>复制</a>"
								  +"</pre></div>";
	...
	for (var i = 0; i < nodeList.length; i++) {
		...
		if (htmlClass.substr(0, 3) === 'sh_') {
			...
			if (language in sh_languages) {
				...
				//console && console.log('Found <pre> element with class="' + htmlClass + '", but no such language exists');
				continue;
			}
			break;
		}
...
/* Shell language (http://shjs.sourceforge.net/lang/sh_sh.min.js) */
